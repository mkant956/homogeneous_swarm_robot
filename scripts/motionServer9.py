...
class MotionServer():
	def __init__(self):
		rospy.init_node('execute_command_server_bane', anonymous = True)
		rospy.Subscriber('front_dist_bane',Float32,self.Callback_distance)
		#-----------------------------------PID CONTROL--------------------------------------------
...
#       	rospy.loginfo("pose : " + str(self.xc) + " " + str(self.yc) + " " + str(self.cur_bear))
	...
	def moveRobot(self,req):
		...
		self.rotateSubscribe = rospy.Subscriber('pose_bane', String, self.callback)
