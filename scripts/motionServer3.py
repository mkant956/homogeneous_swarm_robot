...
isRotate=True
isTranslate=False
def hasDeviated(xc,yc,xt,yt,cur_bear,target):
	rot = abs(cur_bear-target)>10 and abs(abs(cur_bear-target)-360 )>10
	trans = abs((yt-yc)*math.cos(cur_bear) - (xt-xc)*math.sin(cur_bear)) > 0.05

	#return (rot and trans)
	return rot
def callback(data):
	read = data.data
	s1=read.find(' ')
	...
	if(isRotate == False and isTranslate == False):
		...
		#target=targets[n]
               	if(not decideDir(cur_bear,targets[n])):
                       	left(pwm_turn_l_l,pwm_turn_l_r)
                       	c=c+1
                else:
                        right(pwm_turn_r_l,pwm_turn_r_r)
		...
		if(hasDeviated(xc,yc,xt,yt,cur_bear,target)):
			...
			forward(80,80)				#specific for bat
		else:
			stop()
...
def moveRobot(req):
	...
	rotateSubscribe = rospy.Subscriber('pose_bane', String, callback)
	while(execute==0):
		time.sleep(0.1)
	execute = 0
	rotateSubscribe.unregister()
...
def execute_command_server():
    rospy.init_node('execute_command_server_bane')
    s = rospy.Service('execute_command_bane', ExecuteMotionPrimitive, moveRobot)
