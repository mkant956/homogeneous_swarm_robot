...
def moveRobot(req):
	...
	rotateSubscribe = rospy.Subscriber('pose_bat', String, callback)
	while(execute==0):
		time.sleep(0.1)
	execute = 0
	rotateSubscribe.unregister()
...
def execute_command_server():
    rospy.init_node('execute_command_server_bat')
    s = rospy.Service('execute_command_bat', ExecuteMotionPrimitive, moveRobot)
